...
WHOP_ENDPOINT = os.getenv("WHOP_ENDPOINT", "https://api.whop.com/api/v1/forum_posts")
WHOP_ENABLED = os.getenv("WHOP_ENABLED", "true").lower() in ("1", "true", "yes")
WHOP_DRY_RUN = os.getenv("WHOP_DRY_RUN", "true").lower() in ("1", "true", "yes") # Default dry-run
WHOP_DIAGNOSTIC = os.getenv("WHOP_DIAGNOSTIC", "false").lower() in ("1", "true", "yes")

async def send_whop_alert(title: str, content: str) -> bool:
    if not WHOP_ENABLED:
        logger.debug("Whop alerts are disabled.")
        return False

    if not WHOP_API_KEY:
        logger.warning("WHOP_API_KEY is not set. Skipping Whop alert.")
        return False
    ...
    if WHOP_DRY_RUN:
        logger.info("WHOP_DRY_RUN is enabled; simulating post.")
        logger.info(f"Would post to: {WHOP_ENDPOINT}")
        logger.info(f"Experience: {WHOP_EXPERIENCE_ID}")
        logger.info(f"Title: {title}")
        if WHOP_DIAGNOSTIC:
            payload = {"experience_id": WHOP_EXPERIENCE_ID, "title": title, "content": content}
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(
                WHOP_ENDPOINT,
                headers={
                    "Authorization": f"Bearer {WHOP_API_KEY.strip()}",
